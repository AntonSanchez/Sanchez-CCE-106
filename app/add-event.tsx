...
export default function AddEventScreen() {
  ...
  const handleCreate = () => {
    ...
    // Show a brief confirmation, then jump to the new event's detail screen
    // so the student sees it was added.
    setShowToast(true);
    setTimeout(() => {
  ...
  const isFormEmpty =
    title.trim().length === 0 && date.trim().length === 0 && location.trim().length === 0;
    ...
    <KeyboardAvoidingView style={styles.flex} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <Pressable
          ...
          style={[
            styles.createButton,
            isPressed && !isFormEmpty && styles.createButtonPressed,
            isFormEmpty && styles.createButtonDisabled,
          ]}
        >
